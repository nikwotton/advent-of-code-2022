...
fun main() {
    ...
    require(step1Sample == step1SampleExpected) { "Failed sample in step 1, got $step1Sample, got $step1SampleExpected" }
    println("Step 1 answer: ${runStep1(input1)}")
    val step2Sample = runStep2(sample)
    require(step2Sample == step2SampleExpected) { "Failed sample in step 2, got $step2Sample, got $step2SampleExpected" }
